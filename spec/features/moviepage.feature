...
機能: 再生画面を開く
  ...
  シナリオ: sm13の再生画面に表示されているふせんは、ユーザーごとにまとまっている
    もし bookmark_user のテストデータを作成する
    かつ user のテストデータを作成する
    かつ sm13 の再生画面を表示する
    ならば 要素 'div#bookmark h3' が 2 個存在すること

  シナリオ: sm13の再生画面に表示されているふせんは、時間順に並んでいる
    もし bookmark_user のテストデータを作成する
    かつ user のテストデータを作成する
    かつ sm13 の再生画面を表示する
    ならば 要素 'div#bookmark ul li' の 0 番目の要素の内容に 'bookmark_user2_sm13_1' が含まれていること
    かつ 要素 'div#bookmark ul li' の 1 番目の要素の内容に 'bookmark_user2_sm13_2' が含まれていること
    かつ 要素 'div#bookmark ul li' の 2 番目の要素の内容に 'bookmark_user1_sm13_1' が含まれていること
