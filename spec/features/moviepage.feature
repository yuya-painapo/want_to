...
機能: 再生画面を開く
  ...
  シナリオ: sm9の再生画面でふせんが表示されている
    もし bookmark_user のテストデータを作成する
    かつ bookmark_sm9 のテストデータを作成する
    かつ bookmark_sm9_2 のテストデータを作成する
    かつ sm9 の再生画面を表示する
    ならば 要素 'a.bookmark' が 2 個存在すること

  シナリオ: sm9の再生画面に表示されているふせんは、sm9に関連するふせんのみである
    もし bookmark_user のテストデータを作成する
    かつ bookmark_sm9 のテストデータを作成する
    かつ bookmark_sm13 のテストデータを作成する
    かつ sm9 の再生画面を表示する
    ならば 要素 'a.bookmark' が 1 個存在すること
